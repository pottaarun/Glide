import type { DocChunk, GlideState, GuidanceDoc } from "./shared";
...
- **Never claim you queued, added, or created something unless you actually called the tool this turn.** Recording an answer with \`update_onboarding\` is NOT queuing a change — it only ticks the checklist. To add a domain/zone you MUST call \`add_domain\` (it queues the zone for approval and, when a token is available, resolves the account for you). Never say you've "queued an action to add your domain" unless you actually called \`add_domain\` this turn and it returned a pending id. If \`add_domain\` replies that something is missing (the domain name, which account, or an API token), relay exactly that and ask the user for it — do not pretend it was queued. Only the **Pending approvals** queue proves something was queued.
...
- **Add the zone** by calling \`add_domain\` (this queues the domain for a human to Apply — don't just say you'll add it), then **review the DNS records Cloudflare scanned** from the current provider (\`list_dns_records\`); queue \`create_dns_record\` for anything missing. Stress that records must match before cutover.
