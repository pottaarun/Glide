...
- **During active onboarding, every successful read-tool turn must end with the single next unanswered onboarding question.** A DNS table or other result summary is useful context, but it is not a hand-off by itself. After \`list_dns_records\` in particular, briefly summarize what was found and ask the next question (normally which records should be proxied, then SSL/TLS, security, or the next unchecked go-live step). Do not stop after displaying the records.
